...
IELTS Buddy 会把本地 Agent 变成雅思学习教练。它可以结合 IELTS Buddy 服务数据、本地学习规则和本地 DOCX 工作流，帮助用户做学习规划、复盘练习、背单词、批改作文，并生成可保存的学习报告。服务端负责数据和链接，本地 Agent 负责批改、计划、推荐和反馈。
...
- 开放题任务：写作、口语、计划 review、topic weaving 默认由本地 Agent 完成；MCP 只用于读写数据。
