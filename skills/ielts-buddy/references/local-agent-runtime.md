...
| --- | --- | --- |
| Writing Task 1 首轮批改 | 直接使用现有 `workflows/ielts-task1-review/SKILL.md`；不要在本文件复制批改步骤 | 读取题目/历史；保存用户提交的原文、本地评分摘要、DOCX 元数据或学习事件 |
| Writing Task 2 首轮批改 | 直接使用现有 `workflows/ielts-task2-review/SKILL.md`；不要在本文件复制批改步骤 | 同上 |
| Writing 二改 | 直接使用现有 `workflows/writing-revision-loop/SKILL.md`；不要另写二改流程 | 保存 revision 文本、本地 revision report 摘要或学习事件 |
...
| Study plan draft/review | `references/planning.md`：本地读取画像、路线、进度后起草、审查、改计划 | 创建/更新本地 Agent 已写好的计划记录和任务状态 |
| Speaking topic weaving | `workflows/speaking-topic-builder/SKILL.md`：本地从 learner story、目标 part、题库数据中选择 exact questions 并写素材 | 保存本地已选 question links、story、chunks、练习顺序；不能让服务端自动匹配 |
| Reading lexicon | `workflows/ielts-reading-lexicon/SKILL.md`：本地抽词、批注、例句、复习提示并生成 DOCX | 可选保存词条和复习进度 |
| Resource recommendation | `workflows/learning-resource-recommender/SKILL.md`：本地从内置目录筛选并给使用方法 | 不需要云端 AI |
